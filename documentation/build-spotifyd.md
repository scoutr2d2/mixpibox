> **Dieser Weg gilt nicht mehr — VERWORFEN.** MixPiBox spielt Spotify über
> **librespot** ab, nicht über spotifyd; spotifyd ist eine tote Alt-Unit, die das
> OS-Update sogar abschaltet. Diese Datei ist eine unveränderte Fremdkopie der
> spotifyd-Anleitung und bleibt nur als Beleg der Herkunft liegen. Wer hier baut,
> baut etwas, das die Box nicht startet.
>
> Entscheidung: `BACKLOG.md`, Abschnitt „Verworfen — nicht erneut recherchieren"
> (`build-spotifyd.md-Pfad`). Wie librespot auf der Box gestartet wird, steht in
> `scripts/librespot/librespot-start.sh`; ein eigenes Bau-Skript für librespot
> gibt es hier nicht, die Binaries liegen fertig unter `bin/`.
> Dasselbe gilt für `dev/compile_scripts/spotifyd.sh` — gleicher toter Weg.
