...
# ══ DIESE FUNKTION HOLTE DIE ENTSPERRTE UPSTREAM-VERWALTUNG ═════════════════
#
# Sie machte `sudo rm -R /var/www/*` und legte danach www.zip von
# github.com/splitti/MuPiBox hinein. Das ist nicht bloss „fremder Code": in
# DIESER Verwaltung sind die Update-Knoepfe verriegelt
# (AdminInterface/www/includes/herkunftsschutz.php). Die Upstream-Fassung ist
# es nicht — wer die Funktion aufrief, machte den Riegel wieder auf.
#
# Sie steht als Text da und nicht als Befehl, damit der naechste, der sie
# sucht, den Grund findet statt nur ein Loch. Wer die PHP-Verwaltung
# erneuern will, nimmt den Weg dieses Baums:
#     python3 tools/ausliefern.py --nur admin      (vom Laptop)
function mupi-update-webinterface() {
	echo "Abgeschaltet. Diese Funktion holte die Verwaltung von splitti/MuPiBox --"
	echo "also die Fassung OHNE die Update-Sperren dieses Forks, nach einem"
	echo "'rm -R /var/www/*'. Der Weg heisst jetzt, am Laptop:"
	echo "    python3 tools/ausliefern.py --nur admin"
	return 1
}
...
function mupi-info() {
	...
	# KEINE UPSTREAM-VERSION MEHR IN DER BEGRUESSUNG. Sie stand hier als
	# "Latest-Version" und war die des ORIGINALS — auf einer Fork-Box eine
	# Einladung, ein Update zu suchen, das diese Box zerlegen wuerde. Was
	# hier laeuft, sagt die Verwaltung unter "Aktualisierung"; sie kennt
	# auch den Unterschied zwischen "neuer" und "fremde Quelle".
	echo -e "${Color_Off}"
...
# ══ DIE DREI UPDATE-ALIASE SIND ABGESCHALTET ════════════════════════════════
#
# Hier stand dreimal dieselbe Zeile, nur mit anderem Kanal:
#     cd; curl -L …splitti/MuPiBox/main/update/start_mupibox_update.sh \
#         | sudo bash -s -- stable
#
# WAS EIN EINZIGES TIPPEN GEKOSTET HAETTE, nachgelesen im Upstream-Stand
# (Remote `upstream/main`, 3b1a49c7 vom 07.05.2026):
#   * Zeile 274 `rm -R /home/dietpi/.mupibox/Sonos-Kids-Controller-master/` —
#     gerettet werden GENAU DREI Dinge (data.json, www/cover,
#     active_theme.css). resume.json, Profile, Kinderzeit, Akkuverlauf: weg.
#     Im Backlog steht das als F8.
#   * Zeile 425 ueberschreibt /usr/bin/librespot mit einem Stand, den dieser
#     Baum im A/B-Vergleich (28.07.2026) als STUMM belegt hat. Das ist keine
#     Unterlassung, sondern eine aktive Ruecksetzung.
#   * Und die Box liefe danach den Code des Originals, nicht MixPiBox.
#
# KEINE SICHERUNG DAVOR: Die Vor-Update-Sicherung steht in der Fassung
# DIESES Baums. Ueber `curl | sudo bash` laeuft sie nicht mit. Was dann noch
# rettet, haengt nicht am Skript, sondern an der Datei
# (mupibox-sicherung.path) — ein Netz, kein Rueckweg.
#
# WARUM SIE NICHT EINFACH GELOESCHT SIND: Ein fehlender Alias ist ein
# „command not found", und der naechste legt ihn wieder an. Eine Zeile, die
# sagt WARUM, wird nicht wieder angelegt.
mupi-update-stable()  { _mixpi_update_abgeschaltet stable; }
mupi-update-beta()    { _mixpi_update_abgeschaltet beta; }
mupi-update-dev()     { _mixpi_update_abgeschaltet dev; }
_mixpi_update_abgeschaltet() {
	echo "Abgeschaltet -- und zwar mit Absicht."
	echo
	echo "Dieser Befehl holte das Update-Skript von splitti/MuPiBox und fuehrte"
	echo "es als root aus. Es haette:"
	echo "  * MixPiBox durch das Original ersetzt,"
	echo "  * die Nutzerdaten geloescht (gerettet werden dort drei Dateien),"
	echo "  * und librespot auf einen Stand gesetzt, der auf dieser Box STUMM ist."
	echo
	echo "Der Weg fuer neue Fassungen laeuft am Laptop:"
	echo "    python3 tools/ausliefern.py --nur www      (Oberflaeche)"
	echo "    python3 tools/ausliefern.py                (alles ausser scripts)"
	return 1
}
