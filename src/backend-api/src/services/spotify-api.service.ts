...
export class SpotifyApiService {
  ...
  // PKCE mode (no client secret): the SDK client is (re)built from a minted user
  // access token in ensureApi(); pkceExpiresAt tracks when to mint a fresh one.
  private pkce = false
  private pkceExpiresAt = 0
  ...
  constructor(private config: ServerConfig) {
    const sp = this.config.spotify
    if (sp?.clientSecret) {
      // Classic client-credentials flow: an app token, managed by the SDK.
      this.spotifyApi = SpotifyApi.withClientCredentials(sp.clientId || '', sp.clientSecret)
      console.info('Spotify API service initialized (client-credentials)')
    } else if (sp?.refreshToken && sp?.clientId) {
      // PKCE flow (no secret): mint a user token from the refresh token lazily in
      // ensureApi() — this is what makes album/show metadata work under PKCE.
      this.pkce = true
      console.info('Spotify API service initialized (PKCE refresh token)')
    } else {
      // No usable credentials — build a placeholder so callers get clean errors.
      this.spotifyApi = SpotifyApi.withClientCredentials(sp?.clientId || '', sp?.clientSecret || '')
      console.info('Spotify API service initialized (no Spotify credentials configured)')
    }
  }

  /**
   * PKCE flow: ensure a valid SDK client backed by a fresh user access token.
   * Mints one from the stored refresh token (grant_type=refresh_token + client_id,
   * NO client secret) and rebuilds the SDK client when the current token is missing
   * or about to expire. No-op for the client-credentials flow (the SDK manages that
   * token itself) and while the current PKCE token is still valid — so it is cheap
   * to call before every request.
   */
  private async ensureApi(): Promise<void> {
    if (!this.pkce) return
    if (this.spotifyApi && Date.now() < this.pkceExpiresAt - 60_000) return
    ...
    if (!resp.ok) {
      ...
      throw new Error(`Spotify PKCE token refresh failed: ${resp.status} ${txt}`.trim())
    }
    ...
    this.pkceExpiresAt = Date.now() + expiresIn * 1000
    // Spotify may (rarely) rotate the refresh token — keep the newest in memory.
    if (tok.refresh_token && this.config.spotify) {
      this.config.spotify.refreshToken = tok.refresh_token
    }
    this.spotifyApi = SpotifyApi.withAccessToken(sp?.clientId || '', {
      access_token: tok.access_token,
      token_type: tok.token_type || 'Bearer',
      expires_in: expiresIn,
      refresh_token: tok.refresh_token || sp?.refreshToken || '',
    })
  }
  ...
  private async rateLimitedRequest<T>(operation: () => Promise<T>): Promise<T> {
    // PKCE: make sure the SDK client has a fresh user token before any real call
    // (covers both the synchronous queue and the deferred background updates).
    ...
    return this.executeWithCache(
      ...
      async () => {
        const result = await this.spotifyApi.playlists.getPlaylistItems(
          playlistId,
          'DE',
          // The player reads track.album.images/id/name/total_tracks and
          // track.artists[0].name. Asking for id/uri/name alone leaves
          // track.album undefined, so the album view shows no cover art, no
          // album name and no artist at all. Same field list the frontend
          // already uses in spotify-auth.service.ts (getPlaylistDecomposition).
          'items(track(id,uri,name,artists(name),album(id,name,images,total_tracks)))',
          // Spotify allows up to 100 here, and the frontend pages in steps of
          // 50: it stops as soon as a page returns fewer than 50 items. Capping
          // at 10 therefore truncated every playlist after the tenth track.
