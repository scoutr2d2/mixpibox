// Parsers for mplayer slave-mode `ANS_<prop>=<value>` output lines.
// Pure + typed — the first module converted as part of the B2 TypeScript
// migration (no DOM, no I/O; unit-tested in parsers.spec.ts).

type Parser = (raw: string) => unknown

const id = (val: string): string => val
const parseString = (str: string): string => str.slice(1, -1)
const parseFlag = (str: string): boolean => str.toLowerCase().trim() === 'yes'

const knownMetaProps = ['Title', 'Artist', 'Album', 'Year', 'Comment', 'Genre']

const parseStringList = (str: string): Record<string, string> => {
  const res: Record<string, string> = Object.create(null)
  const parts = str.split(',')
  let metaProp: string | null = null
