...
# LAUTSTAERKE UEBER DEN EINEN WEG (BACKLOG E12/X7).
#
# Hier stand zweimal `amixer sset Master <n>%`, hart und ohne Ruecklaufpruefung.
# Zwei Dinge waren daran falsch:
#
# 1. DIE OBERGRENZE WURDE UMGANGEN. `mupibox.maxVolume` steckt seit dem
#    01.08.2026 in mupi-lautstaerke.sh (Funktion klemme()), und der
#    Abspieldienst geht dort durch. Telegram ging daran vorbei — ein Kind mit
#    dem Telefon der Eltern konnte die Box lauter stellen, als die Eltern
#    erlaubt hatten.
# 2. DIE MELDUNG WAR EINE FALSCHAUSKUNFT. `subprocess.run` ohne `check` wirft
#    nichts; "Volume set to 80%" ging auch dann heraus, wenn gar nichts
#    passiert war. Eine fehlende Wirkung ist aergerlich, eine falsche Auskunft
#    laesst jemanden woanders suchen.
#
# DER RUECKFALL BLEIBT: fehlt das Werkzeug (alte Box, halbes Update), wird die
# Box wie bisher ueber amixer geregelt — seit E131/B5 MIT der Obergrenze:
# dieselbe Skalenrechnung wie in chromium-autostart.sh (Wert x maxVolume/100),
# sonst waere der Rueckfall genau der Zwei-Skalen-Fehler aus E130 in
# Wartestellung. Dasselbe Muster wie in scripts/mqtt/mqtt.py.
LAUTSTAERKE = "/usr/local/bin/mupibox/mupi-lautstaerke.sh"


def max_volume():
    """mupibox.maxVolume als Zahl 1..100; alles Unlesbare heisst 100."""
    try:
        roh = int(str(config["mupibox"]["maxVolume"]).strip())
        return min(max(roh, 1), 100)
    except (KeyError, ValueError, TypeError):
        return 100


def lautstaerke_setzen(wert):
    """Setzt die Lautstaerke (0-100, Nutzerskala) und sagt, ob es geklappt hat."""
    wert = str(wert).strip().rstrip('%')
    if not wert.isdigit():
        return False
    if os.access(LAUTSTAERKE, os.X_OK):
        return subprocess.run([LAUTSTAERKE, "set", wert]).returncode == 0
    echt = min(int(wert), 100) * max_volume() // 100
    return subprocess.run(
        ["/usr/bin/amixer", "-q", "sset", "Master", str(echt) + "%"]
    ).returncode == 0
...
def on_chat_message(msg):
    ...
    if command == '/shutdown':
        ...
        volume = split_cmd[1]
        if lautstaerke_setzen(volume):
            bot.sendMessage(chat_id, "Volume set to " + volume + "%")
        else:
            bot.sendMessage(chat_id, "Volume NOT set (" + volume + ")")
        ...
        # 127.0.0.1 statt mupibox.host — Begruendung wie in scripts/mqtt/mqtt.py:
        # der Abspieldienst hoert nur noch auf der Rueckschleife.
...
def on_callback_query(msg):
    ...
    if query_data == 'screen':
        ...
        if lautstaerke_setzen(volume):
            ...
            bot.answerCallbackQuery(query_id, text='Volume NOT set (' + volume + ')', show_alert=True)
