...
def setup_logging(logfile):
    ...
    STUFE WARNING, NICHT INFO (E48, 19.08.2026).

    `log_register_values()` schreibt 21 Zeilen je Durchlauf, und der Durchlauf
    kommt alle 4 Sekunden: gemessen 168 Zeilen je Minute und 35,6 MB Protokoll,
    die niemand liest. Das ist auf dieser Box kein Schoenheitsfehler, sondern
    Arbeitsspeicher — dietpi-ramlog legt /var/log auf ein 50-MB-tmpfs, und von
    2010 MB waren 197 MB frei.

    Die Registerwerte sind FEHLERSUCHE, nicht Betrieb. Wer sie braucht, setzt
    MUPIHAT_LOGLEVEL=INFO (oder DEBUG) und startet den Dienst neu; Warnungen
    und Fehler stehen weiterhin immer da. Der Betriebszustand, auf den es
    ankommt, liegt ohnehin in /tmp/mupihat.json — dort liest ihn der Server.
    """
    stufe = os.environ.get("MUPIHAT_LOGLEVEL", "WARNING").upper()
    logging.basicConfig(
        filename=logfile,
        level=getattr(logging, stufe, logging.WARNING),
...
def main():
    ...
    # Flask web server — NUR AUF DER SCHLEIFE (E48, 19.08.2026)
    #
    # Hier stand host="0.0.0.0": die Registeranzeige des Ladechips horchte auf
    # ALLEN Netzkarten (gemessen: LISTEN 0 128 0.0.0.0:5000). Gebraucht wird
    # sie zur Fehlersuche, und der dokumentierte Weg dorthin ist ohnehin ein
    # Tunnel (`mupictl --device <host> forward`, mupictl:75) — der arbeitet
    # mit 127.0.0.1 unveraendert weiter.
    #
    # NICHT ENTFERNT, nur gebunden: app.run IST die Hauptschleife dieses
    # Dienstes. Wer sie streicht, nimmt der Box auch den periodischen
    # /tmp/mupihat.json-Schreiber, an dem die Tiefentlade-Abschaltung haengt.
    try:
