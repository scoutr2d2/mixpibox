...
def mqtt_publish_ha():
    ...
    # Publish Fan speed state entity
    # WAR: Plattform "fan" - die verlangt in Home Assistant einen STEUERBAREN
    # Luefter (command_topic), den es hier nie gab. Ohne command_topic bleibt
    # die Entitaet kaputt/nicht anlegbar. Es ist ein Messwert (PWM-Tastgrad in
    # %, siehe fan_control.py) - dafuer ist "sensor" die richtige Plattform,
    # wie bei den Nachbarbloecken (Vorbild: signal_quality_info).
    fan_info = {
        "name": "Fan speed",
    ...
    client.publish(mqtt_ha_topic + "/sensor/" + mqtt_clientId + "_fan/config", json.dumps(fan_info), qos=0, retain=False)
...
# paho-mqtt 1.x ruft on_connect(client, userdata, flags, rc) auf; ab 2.0 mit
# CallbackAPIVersion.VERSION2 (siehe main(), Rang 8 AUDIT-2026-08-29: Trixie
# liefert paho 2.x, Bestandsboxen auf Bookworm noch 1.6) kommt zusaetzlich
# `properties` dazu, und `rc` heisst dort `reason_code`. `*extra` traegt
# beide Formen; die Position des Codes ist der einzige Unterschied:
# v1 (..., rc) - Code ganz am Ende. v2 (..., reason_code, properties) - Code
# an vorletzter Stelle, weil danach noch properties kommt.
def _reason_code(extra):
    if not extra:
        return None
    return extra[-2] if len(extra) >= 3 else extra[-1]

# Callback function for when the client receives a CONNACK response from the server.
def on_connect(client, userdata, *extra):
    print("Connected to MQTT broker with result code: " + str(_reason_code(extra)))

# Callback function for when the client is disconnected from the broker
def on_disconnect(client, userdata, *extra):
    print("Disconnected from MQTT broker with result code: " + str(_reason_code(extra)))
    # HIER STAND FRUEHER ein Publish von "offline" ueber denselben Client -
    # der ist an dieser Stelle aber schon getrennt, die Nachricht ging also
    # ins Leere. Der richtige Mechanismus dafuer ist ein Last Will
    # (client.will_set(...) VOR client.connect(), siehe main()): den
    # veroeffentlicht der BROKER, sobald er die Verbindung als tot erkennt -
    # unabhaengig davon, ob dieser Prozess dann noch etwas tun kann.
...
def get_volume():
    # UEBER mupi-lautstaerke.sh, NICHT MEHR DIREKT UEBER alsaaudio.Mixer()
    # (Rang 8, AUDIT-2026-08-29). alsaaudio.Mixer() liest immer den ALSA-Mixer
    # "Master" - auf einer PipeWire-Box (E12/X6) ist das aber nicht die
    # Stufe, die tatsaechlich klingt. Ergebnis: das LESEN hier zeigte einen
    # anderen Wert als das SCHREIBEN weiter unten (volume/set), das schon
    # ueber dieses Skript geht. Gleicher Weg fuer beide Richtungen, gleicher
    # Rueckfall wie dort: fehlt/ist das Skript nicht ausfuehrbar, alter Pfad.
    werkzeug = "/usr/local/bin/mupibox/mupi-lautstaerke.sh"
    if os.access(werkzeug, os.X_OK):
        ausgabe = subprocess.check_output([werkzeug, "get"], universal_newlines=True).strip()
...
def get_fan():
    try:
        ...
        # fan_control.py schreibt "100%" — mit Prozentzeichen. int("100%")
        # wirft, das bare except schluckte es, und die Entitaet blieb IMMER
        # leer (gefunden 29.08.2026 beim sensor-Umbau). Wie bei Bat_SOC in
        # get_mupihat(): erst das Zeichen weg, dann die Zahl.
        return int(str(fan_data["speed"]).replace("%", "").strip())
    except:
        return None
    ...
    try:
        ...
        if local and state:
            if local != previous_content_local or state != previous_content_state:
                ...
                # Der Screenshot (sudo-scrot auf dem Kiosk-X) gehoert hinter
                # dieselbe Aenderungspruefung wie play_text: frueher lief er
                # bei JEDEM Schleifendurchlauf, publiziert wurde er aber nur
                # hier drin - unnoetiger sudo/X-Griff bei jedem Takt, und der
                # Screenshot war zudem einen Durchlauf alt (er entstand ERST
                # NACH diesem Aufruf). Jetzt entsteht er unmittelbar vor dem
                # Lesen, nur wenn sich wirklich etwas geaendert hat.
...
def get_screenshot():
    try:
        ...
        print("Exception: ", e)
        # `null` gibt es in Python nicht — das war ein NameError im
        # Fehlerpfad, den erst der try/except des Aufrufers auffing.
        return None
...
def on_message(client, flags, msg):
    ...
    if msg.topic == mqtt_topic + '/' + mqtt_clientId + '/volume/set':
        # UEBER mupi-lautstaerke.sh, NICHT MEHR DIREKT UEBER pactl (E12/X7).
        #
        # Hier stand `os.system("/usr/bin/pactl set-sink-volume @DEFAULT_SINK@ "
        # + nutzlast + "%")`. Zwei Dinge daran:
        #
        # 1. Die Obergrenze `mupibox.maxVolume` wurde umgangen — sie steckt in
        #    mupi-lautstaerke.sh, und MQTT ging daran vorbei. Wer die Box ueber
        #    Home Assistant bedient, hatte keine Obergrenze.
        # 2. Die Nutzlast ging UNGEPRUEFT in eine Shell-Zeile. Eine Nachricht
        #    wie `50%; reboot` waere ausgefuehrt worden. Jetzt wird sie erst
        #    auf Ziffern geprueft und dann als Einzelwort uebergeben — ohne
        #    Shell dazwischen.
        wert = msg.payload.decode("utf-8").strip().rstrip('%')
        print("Volume: " + wert + "%")
        if wert.isdigit():
            werkzeug = "/usr/local/bin/mupibox/mupi-lautstaerke.sh"
            if os.access(werkzeug, os.X_OK):
                subprocess.run([werkzeug, "set", wert])
            else:
                # Rueckfall MIT Obergrenze (E131/B5): dieselbe Skalenrechnung
                # wie chromium-autostart.sh und telegram_receiver.py — der
                # Nutzerwert 0..100 wird auf 0..maxVolume der Karte
                # abgebildet. Ohne sie waere dieser Pfad der Zwei-Skalen-
                # Fehler aus E130 in Wartestellung.
                try:
                    with open("/etc/mupibox/mupiboxconfig.json") as f:
                        maxv = int(str(json.load(f)["mupibox"]["maxVolume"]).strip())
                    maxv = min(max(maxv, 1), 100)
                except (OSError, KeyError, ValueError, TypeError):
                    maxv = 100
                echt = min(int(wert), 100) * maxv // 100
                subprocess.run(["/usr/bin/pactl", "set-sink-volume",
                                "@DEFAULT_SINK@", str(echt) + "%"])
        else:
            print("Volume: Nutzlast ist keine Zahl - nichts gesetzt")
    if msg.topic == mqtt_topic + '/' + mqtt_clientId + '/pause/set' and str(msg.payload.decode("utf-8")) == "pause":
        print("Button: pause")
        # 127.0.0.1 UND NICHT mupibox.host: der Abspieldienst hoert seit dem
        # 14.08.2026 nur noch auf der Rueckschleife (spotify-control.ts). Ueber
        # den Hostnamen ginge der Aufruf ins Leere — und zwar leise, denn ein
        # Fehlschlag steht nur im Journal. Dieses Skript laeuft AUF der Box.
...
#        pid_file.write(str(pid))
    ...
    if check_server_availability(mqtt_broker,1883,mqtt_refresh):
        ...
        #
        # Versions-Weiche paho 1.x/2.x (Rang 8, AUDIT-2026-08-29): ab 2.0
        # verlangt der Client eine CallbackAPIVersion, sonst eine
        # DeprecationWarning (2.x) bzw. einen TypeError (kuenftige 3.x).
        # `mqtt.CallbackAPIVersion` gibt es erst ab 2.0 - ueber dessen
        # Existenz erkennen wir die Version, ohne eine bestimmte
        # paho-Version vorauszusetzen. on_connect/on_disconnect vertragen
        # oben ueber `_reason_code()` beide Aufrufformen.
        ...
        # Last Will: siehe on_disconnect() weiter oben - MUSS vor connect()
        # gesetzt werden, der Broker nimmt sie nur beim Verbindungsaufbau an.
        client.will_set(mqtt_topic + '/' + mqtt_clientId + '/state', payload="offline", qos=0, retain=False)
        ...
        try:
            while True:
                ...
                client.publish(mqtt_topic + '/' + mqtt_clientId + '/volume', get_volume(), qos=0)
